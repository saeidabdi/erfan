...
<div class="content container main_box" v-if="logined">
    <div class="div col-lg-9 left">
        ...
        <div class="col-md-12" v-if="!order_id" style="padding-top: 30px;">
            <ul class="ul_allorder">
                <li class="order_item" v-for="order in all_order">
                    <h4 class="right" style="cursor: pointer;" @click="get_inorder(order.id,order.title)">@{{order.title}}</h4>
                    <div class="right time">قیمت نهایی : @{{order.price + order.price_service}} تومان</div>
                    <span class="right time">زمان ثبت سفارش : @{{order.time_added}}</span>
                    <div class="left">
                        <button type="button" v-if="order.status == 0 || order.status == 1" class="btn btn-danger" @click="delete_order(order.id)">حذف</button>
                        ...
                        <button v-if="order.status == 3" @click="get_inorder(order.id,order.title)" type="button" class="btn btn-info">آماده تحویل</button>
                    </div>
                </li>
            </ul>
        </div>
        ...
        <div class="col-md-12" v-if="order_id" style="padding-top: 10px;">
            ...
            <div class="col-md-12 right" v-if="order[0].status == 3">
                <div class="alert alert-success" role="alert" style="text-align: center;">
                    سفارش شما آماده تحویل میباشد جهت دریافت با شماره 09028545707 تماس بگیرید
                </div>
            </div>
